...
# Where the test server's stdout/stderr is written (see the fastapi_server fixture).
SERVER_LOG_PATH = "test-server.log"
...
def fastapi_server():
    ...
    # Send the server's output to a log file rather than a pipe. Nothing drains
    # the pipes while the tests run, so a long browser suite eventually fills
    # the OS pipe buffer and uvicorn blocks on write - the server appears to
    # hang and page navigations time out. Writing to a file never blocks.
    ...
    def read_server_log() -> str:
        """Read back whatever the server has written so far."""
        try:
            server_log.flush()
    ...
    if not wait_for_server(health_url, timeout=30):
        logger.error(f"Server failed to start. Uvicorn output:\n{read_server_log()}")
        process.terminate()
        server_log.close()
        raise ServerStartupError(f"Failed to start test server on {health_url}")

    logger.info(f"Test server running on {server_url}. Server log: {SERVER_LOG_PATH}")
    yield server_url
